...
namespace MultiThreading.Task3.MatrixMultiplier
{
    class Program
    {
        ...
        {
            byte matrixSize = default;
            bool matrixSizeIsValid = false;
            Console.WriteLine("3.	Write a program, which multiplies two matrices and uses class Parallel. ");
            Console.WriteLine();
        
            while(!matrixSizeIsValid)
            {
                Console.WriteLine("Enter valid matrix size.");
                string eneteredSize = Console.ReadLine();

                matrixSizeIsValid = byte.TryParse(eneteredSize, out matrixSize);            
            }
